...
def main():
    ...
    if "usuario" in st.session_state:
        ...
        # Menu Principal (inclui "Gestão de Leads")
                ...
                if st.form_submit_button("Salvar Cliente"):
                    if not nome or not email or not telefone or not endereco:
                        ...
                        novo_cliente = {
                            ...
                            "aniversario": aniversario.strftime("%Y-%m-%d"), "endereco": endereco,
        ...
        # ------------------ Relatórios ------------------ #
            ...
            with st.expander("🔍 Filtros Avançados", expanded=True):
                with st.form("form_filtros"):
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        # Incluímos "Leads" como opção de relatório
                    ...
                    if st.form_submit_button("Aplicar Filtros"):
                        if tipo_relatorio == "Processos":
                            dados_filtrados = aplicar_filtros(PROCESSOS, {"area": area_filtro, "escritorio": escritorio_filtro, "responsavel": responsavel_filtro, "data_inicio": data_inicio, "data_fim": data_fim})
                            ...
                            dados_filtrados = aplicar_filtros(ESCRITORIOS, {"data_inicio": data_inicio, "data_fim": data_fim})
                            st.session_state.dados_relatorio = dados_filtrados
                            st.session_state.tipo_relatorio = "Escritórios"
                        else:  # Leads
                            # Para Leads, não aplicamos filtros avançados; usamos os dados carregados
